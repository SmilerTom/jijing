...
Use the pure functions on every render. Show cash, holding value, total assets, P/L, average cost, break-even NAV, and required rise. Render both entry and exit ledgers with the shared columns `节点 / 幅度 / 金额 / 累计 / 剩余 / 份额 / 持仓市值 / 总资产 / 净值 / 收益率`; make decline and rebound controls native selects with 5%–50% integer options. For exits, expose both editable sell ratio and sell shares, with manual shares taking precedence and the ratio recalculated from current remaining shares. Add a separate recovery block showing current NAV, current assets at the selected scenario, recovery NAV, and recovery rise. Use green `+` for entry amounts, red `−` for exit amounts, and green/red semantic colors for positive/negative returns.
